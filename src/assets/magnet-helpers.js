...
// Store the cube's original rotation
const originalRotation = el.object3D.rotation.clone();
...
// Calculate the necessary rotation to maintain the original world rotation
const parentWorldQuaternionInverse = new THREE.Quaternion().copy(this.el.object3D.getWorldQuaternion(new THREE.Quaternion())).invert();

// Create a Quaternion representing the original rotation
const originalRotationQuaternion = new THREE.Quaternion().setFromEuler(originalRotation);

// Multiply the inverse parent's world quaternion with the original rotation quaternion
const restoredLocalRotationQuaternion = parentWorldQuaternionInverse.multiply(originalRotationQuaternion);

// Convert the restored rotation quaternion to Euler angles
const restoredLocalRotation = new THREE.Euler().setFromQuaternion(restoredLocalRotationQuaternion);
...
// Apply the restored local rotation to the cube
el.object3D.rotation.copy(restoredLocalRotation);
